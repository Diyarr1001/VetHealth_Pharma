      ...
      {isAdding ? (
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 max-w-2xl mb-8">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold font-heading text-slate-800">Add New Product</h2>
          ...
          <form onSubmit={handleCreateProduct} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input required type="number" step="0.01" className="w-full border border-slate-200 rounded-lg p-2" value={formData.price} onChange={e => setFormData({...formData, price: parseFloat(e.target.value)})} />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Category UUID (Optional for now)</label>
                <input type="text" className="w-full border border-slate-200 rounded-lg p-2" placeholder="Leave empty if unknown" value={formData.categoryId} onChange={e => setFormData({...formData, categoryId: e.target.value})} />
              </div>
            </div>
            ...
            <div className="pt-4 flex justify-end gap-3">
              <button type="button" onClick={() => setIsAdding(false)} className="px-5 py-2 text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg font-medium transition-colors">Cancel</button>
